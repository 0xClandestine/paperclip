...
export function NewAgent() {
  ...
  const effectiveRole = isFirstAgent ? "explorer" : role;
  ...
  useEffect(() => {
    if (isFirstAgent) {
      if (!name) setName("Researcher");
      if (!title) setTitle("Researcher");
    }
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <div className="border border-border">
        ...
        <div className="border-t border-border px-4 py-3">
          {isFirstAgent && (
            <p className="text-xs text-muted-foreground mb-2">This will be the first researcher agent</p>
          )}
          {formError && (
