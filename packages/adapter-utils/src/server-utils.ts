...
/**
 * Applies all PAPERCLIP_* context env vars derived from the run context onto `env`.
 * Covers task/issue id, wake reason, comment id, approval id/status, linked issues,
 * wake payload JSON, and issue work mode.
 *
 * Call this after `buildPaperclipEnv()` and before applying user-supplied `config.env`.
 */
export function applyPaperclipContextEnv(
